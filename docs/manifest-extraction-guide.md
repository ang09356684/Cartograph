...
> 本文件是 `manifest-template-essentials.md` 的**實作搭檔**。essentials 定義「寫什麼」，本文件告訴你「值從哪裡挖」。
>
> 涵蓋語言 / 框架：
...
| `sequence_mermaid` | 手繪 / AI 產（必填；完整規則見 essentials §9，本檔 §7 補語言對照） | 同 |
...
**原則**：讀 handler 的邏輯流程，每個「觀察得到的行為」算一步。不要拆太細（變數賦值、log）；也不要太粗（整個 function 一步）。完整欄位規格見 essentials §8 的 step 欄位表。
...
| `action` | 看該區塊做的事，snake_case 動詞片語（對照 essentials §8 的命名慣例） | 同 |
...
| `target_api_ref` | **若此 step 透過 adapter 呼叫另一個 Cartograph-indexed repo 的特定 API 才填**；格式 `<target-repo-id>:<target-api-id>`（例如 `media-svc:audio-convert`）。挖法：看 adapter call 的 URL path（例：`internal/adapter/server/media_server.go` 的常數 `apiAudioConvert = "/api/v1/audio/convert"`）→ 對照目標 repo 的 `apis/<id>.yaml#path` 找出對應 api id。**目標 repo manifest 還沒寫也照填**，aggregator render 時會 degrade 成 disabled pending badge，目標加進來後自動活化。見 essentials §13。 | 同（讀 HTTP client / SDK call 的 URL path） |
...
| 欄位 | Go | Python |
|---|---|---|
| `id` | 平台短名（整合目標是另一個 your-org repo 時，**id 隨便取**即可；跨 repo 連結改用 step 欄位 `target_api_ref` 宣告，不再靠 integration.id 去對齊 repo id — 見 essentials §13） | 同 |
| `kind` | 見 §6.1 表 | 同 |
...
`sequence_mermaid` 是必填欄位、Zod 擋空值，也是**唯一需要手繪**的欄位。

**所有規則**（participant 骨架 / arrow ↔ steps 對應 / 可省略 plumbing / 必須獨立 arrow / 簡短同層合併例外 / DB SQL 粒度 / 分支 alt/else / 巢狀禁止）見 `manifest-template-essentials.md §9`。本節只補 **「從 source code 怎麼挖 arrow」** 的語言對照。

兩個實作技巧：

1. **照著 `steps[]` 畫**：participants = handler + service + 外部依賴；arrows = 每個 step 的 interaction
...
| `insert_row` / `update_row` / `delete_row` | `S->>DB: insert/update/delete <entity>` | 預設 action-phrase；SQL 片段只在 7 個觸發點（soft-delete filter / JOIN / UPSERT / partial unique / cursor / CTE 等）才保留 — 見 essentials §9 |
